...
session_start();
include '../dist/db_connection/connection.php';
          ...
          <body style="font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px; margin: 0;">
            <div style="max-width: 500px; margin: auto; background: #ffffff; border-radius: 8px; padding: 20px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
              ...
              <p style="font-size: 16px; color: #555; text-align: center;">
                Use the following One-Time Password to for TBAQS Security Settings:
              </p>
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($username) || empty($email) || empty($password)) {
        $message = "All fields are required.";
          ...
          $message = "Please use your account username";          
          //header("Location: settings.php");
          //exit();
    } else {
        // Check if user exists in new_registered_user
        ...
        if (sendOtpEmail($email, $otp)) {
            ...
            header("Location: new_pass_otp.php");
            exit();
        } else {
...
if (isset($_FILES['photo']) && $_FILES['photo']['error'] === 0 && $loggedInUser) {
    ...
    $stmt = $conn->prepare("UPDATE new_registered_user SET profile = ? WHERE Username = ?");
    $null = NULL;
...
<!--begin::Row-->
            <div class="row">
              <div class="col-sm-6">
                <h3 class="mb-0" style="font-weight: 990; color: #003366;">Security</h3>
              </div>
              <div class="col-sm-6">
...
<style>
  /* Only style the main section, not the whole page */
  .animated-background {
    position: relative;
    min-height: 100vh;
  ...
  .btn-primary { background-color: #0d3380; border: none; }
...
<div class="animated-background">
  ...
  <div class="settings-card">
    <h1><i class="bi bi-lock-fill"></i> Set Password</h1>
    ...
    <form method="post">
      ...
      <div class="mb-3">
        ...
        <input type="tel" inputmode="numeric" pattern="[0-9]*" class="form-control" id="password" name="password" placeholder="Enter new password" required>
      </div>
